...
#+end_src
** mcordell/gh-pr-fetch-info
Fetches and parses PR JSON from =gh pr view --json= for a given URL.
Returns the parsed hash table, or nil if the request fails or the URL is invalid.
#+begin_src emacs-lisp :tangle autoload/my-org.el
;;;###autoload
(defun mcordell/gh-pr-fetch-info (url)
  ...
  (let* ((json-str (shell-command-to-string
    ...
    (when (and info (hash-table-p info) (gethash "title" info))
      info)))
#+end_src
** mcordell/gh-pr-ci-status
Derives an overall CI status string from the =statusCheckRollup= vector returned by gh.
Handles both =CheckRun= and =StatusContext= types. Returns =SUCCESS=, =FAILURE=, =PENDING=, or =NONE=.
#+begin_src emacs-lisp :tangle autoload/my-org.el
;;;###autoload
  ...
  (if (or (null rollup) (zerop (length rollup)))
    ...
    (let (has-failure has-pending)
      ...
      (cond (has-failure "FAILURE")
            ...
            (t "SUCCESS")))))
#+end_src
** mcordell/gh-pr-properties
Derives an alist of org property key-value pairs from a parsed gh PR info hash table.
Used by both the capture template and the buffer update function to keep property logic in one place.

Properties returned:
- =PR_URL= — raw URL for future lookups
- =PR_AUTHOR= — login of the PR author
- =PR_BRANCH= / =PR_BASE= — branch names
- =PR_STATUS= — OPEN, DRAFT, MERGED, or CLOSED
- =PR_CI= — overall CI result: SUCCESS, FAILURE, PENDING, or NONE
- =PR_DECISION= — APPROVED, CHANGES_REQUESTED, REVIEW_REQUIRED
- =PR_NEXT_ACTION= — AUTHOR, REVIEWER, CI, MERGE, MERGED, CLOSED, or UNKNOWN
- =PR_REVIEWS= — space-separated =login:STATE= pairs from latest reviews
- =PR_PENDING= — space-separated logins of requested-but-not-yet-reviewed people
#+begin_src emacs-lisp :tangle autoload/my-org.el
;;;###autoload
...
Prompts for a GitHub PR URL (defaults to clipboard if it contains a PR URL), fetches
metadata via =mcordell/gh-pr-fetch-info=, and returns a formatted org entry string.
All property logic is delegated to =mcordell/gh-pr-properties=.
#+begin_src emacs-lisp :tangle autoload/my-org.el
;;;###autoload
...
#+end_src
** mcordell/update-pr-statuses-in-buffer
Scans the current buffer for any org heading that has a =PR_URL= property, re-fetches
the PR status from GitHub via =gh=, and updates all properties in place using
=mcordell/gh-pr-properties=. Reports how many headings were updated.
#+begin_src emacs-lisp :tangle autoload/my-org.el
;;;###autoload
