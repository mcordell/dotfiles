...
#+end_src
** mcordell/select-delegate-person
Prompt for a direct report name and return it as a tag-friendly string for use in capture templates.
#+begin_src emacs-lisp :tangle autoload/my-org.el
;;;###autoload
(defun mcordell/select-delegate-person ()
  "Prompt for a person and return as a tag-friendly string."
  (let* ((people (append mcordell/direct-reports
                         mcordell/key-collaborators
                         mcordell/skip-levels
                         mcordell/manager-name))
         (name (completing-read "Delegate to: " people nil t)))
    (mcordell/normalize-name-to-tag name)))
#+end_src
...
")
                                                  ("d" "Delegate" entry (here)
                                                   "* DELG %^{Task} :%(mcordell/select-delegate-person):
%?"
                                                   :empty-lines 1))
